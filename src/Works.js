...
import index_anotheries from './static/img/works/indexs/anotheries.png'
...
const Projects = ({
                    ...
                    descriptionPositonChoice,
                    url,
                    positionFinTuning,
                    id
                  }) => {
  var roleRender = roles.map(item => <div className={worksCss.description_contents_prop}>{item}</div>);
  var typeRender = type.map(item => <div className={worksCss.description_contents_prop}>{item}</div>);
    ...
    for (let i = 0; i < len; i++) {
      document.getElementsByClassName(`${worksCss.project_img_container_prop}`)[i].addEventListener('mouseover',
        () => {
          ...
          document.getElementsByClassName(`${worksCss.description_container}`)[i].style.transition =
            'all .3s ease-in-out';
        })
      document.getElementsByClassName(`${worksCss.project_img_container_prop}`)[i].addEventListener(
        'mouseleave', () => {
  ...
  return (
    <div className={descriptionPositonChoice} style={{marginLeft: positionFinTuning}} id={id}>
      <div className={worksCss.project_img_and_name_container}>
        <Link className={worksCss.project_img_container_prop} style={{width: imgWidth, height: imgHeight}}
              to={url}
              ...
              data-aos-once="false">
          <img src={img} className={worksCss.project_img_prop} style={{width: imgWidth, height: imgHeight}}
               alt={name}></img>
...
export default function Works() {
  ...
  useEffect(() => {
    if (location.state) {
      let pageName = location.state.previousPage.split("/")[2];
    ...
    <>
      ...
      <PageLayout>
        <section id={worksCss.works}>
          <Navbar />
          <Sidebar sidebarTitle="Enjoy this page" emoji_1="⬇️" emoji_1_text="Scroll" emoji_2="👋"
                   emoji_2_text="Hover" style={{top: '0'}} colorEmoji="#F5BC29" />
          ...
          <div className={worksCss.works_container}>
            <div className={worksCss.works_contents_container}>
              <Projects name="にしかげクリニック アネックス" type={["Website", "(Client project with willstyle)"]}
                        ...
                        descriptionContainerHeight="18.29rem"
                        descriptionPositonChoice={`${worksCss.project_container_left}`}
                        url="/works/nishikage"
                        id="nishikage"
              />
              ...
              <Projects name="株式会社じょぶ" type={["Website", "(Client project with willstyle)"]}
                        ...
                        imgWidth="29.27rem" imgHeight="18.29rem"
                        descriptionContainerHeight="18.29rem"
                        descriptionPositonChoice={`${worksCss.project_container_right}`}
                        url="/works/job_homes"
                        id="job_homes"
              />
              ...
              <Projects name="Everything's Gonna Be Alright" type={["Music video"]}
                        ...
                        id="everything_is_gonna_be_alright"
              />
